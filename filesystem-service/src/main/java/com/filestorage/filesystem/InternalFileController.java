...
class InternalFileController {
    ...
    void releaseQuota(@RequestHeader(Headers.INTERNAL_TOKEN) String token,
    ...
    FolderArchiveResponse archiveManifest(@RequestHeader(Headers.INTERNAL_TOKEN) String token,
                                          @PathVariable UUID id,
                                          @RequestParam UUID userId) {
        InternalAuth.requireToken(token, internalToken);
