package com.filestorage.filesystem.service;
...
public class FilesystemService {
    ...
    public FilesystemService(FolderRepository folders,
    ...
    @Transactional
    public FolderResponse provisionRoot(UUID userId) {
   ...
   public  FolderResponse root(UUID userId) {
        FolderNode root = folders.findByOwnerIdAndParentIdIsNull(userId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "ROOT_NOT_PROVISIONED_YET"));
        return new FolderResponse(root.id, root.name, root.parentId);
    }

   public  ChildrenResponse children(UUID userId, UUID folderId) {
   ...
   public  QuotaResponse quota(UUID userId) {
    ...
    @Transactional
    public InternalFileResponse activate(UUID fileId, ActivateFileRequest request) {
        StoredFile file = findFile(fileId);
    ...
    @Transactional
    public void reject(UUID fileId, RejectFileRequest request) {
        StoredFile file = findFile(fileId);
        StorageQuota quota = quotas.findById(file.ownerId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "QUOTA_NOT_PROVISIONED_YET"));
   ...
   public  FolderArchiveResponse archiveManifest(UUID userId, UUID folderId) {
        FolderNode folder = findOwnedFolder(userId, folderId);
    if(isFolderTrashed(folder)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "FOLDER_NOT_FOUND");
        }
    ...
    @Transactional
    public void moveToTrash(UUID userId, ResourceType type, UUID resourceId) {
    switch(type) {
   ...
   public  TrashResponse trash(UUID userId) {
    ...
    @Transactional
    public RestoreResponse restore(UUID userId, ResourceType type, UUID resourceId) {
    ...
    @Transactional
    public void purge(UUID userId, ResourceType type, UUID resourceId, boolean permanent) {
    if(!permanent) {
