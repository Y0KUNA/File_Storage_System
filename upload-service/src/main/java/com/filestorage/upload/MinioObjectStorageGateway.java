...
class MinioObjectStorageGateway implements ObjectStorageGateway {
    ...
    MinioObjectStorageGateway(MinioClient minio,
    ...
    public PresignedPut presignPut(String storageKey) {
        ...
        try {
            String url = minio.getPresignedObjectUrl(GetPresignedObjectUrlArgs.builder()
                    ...
                    .build());
            return new PresignedPut(url, Instant.now().plus(presignExpiryMinutes, ChronoUnit.MINUTES));
        } catch (Exception ex) {
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "MINIO_PRESIGN_PUT_FAILED", ex);
        }
    }
    ...
    public long statObjectSize(String storageKey) {
        ensureBucket();
        try {
            return minio.statObject(StatObjectArgs.builder().bucket(bucket).object(storageKey).build()).size();
        } catch (Exception ex) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "OBJECT_NOT_FOUND_IN_STORAGE", ex);
        }
    }
    ...
    public long completeMultipart(String storageKey, int totalParts) {
        ...
        try {
            ...
            for (int partNumber = 1; partNumber <= totalParts; partNumber++) {
                ...
                sources.add(ComposeSource.builder().bucket(bucket).object(partKey).build());
            }
            minio.composeObject(ComposeObjectArgs.builder()
                    .bucket(bucket)
                    .object(storageKey)
                    .sources(sources)
                    .build());
            ...
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "MINIO_MULTIPART_COMPLETE_FAILED", ex);
        }
    }
