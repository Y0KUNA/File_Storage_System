package com.filestorage.upload.service;
...
public class UploadService {
    ...
    public UploadService(@Value("${app.multipart-threshold-bytes:104857600}") long multipartThresholdBytes,
    ...
    @Transactional
    public CreateUploadResponse create(UUID ownerId, CreateUploadRequest request) {
    ...
    @Transactional
    public ConfirmUploadResponse confirm(UUID fileId) {
   ...
   public  UploadStatusResponse status(UUID uploadSessionId) {
   ...
   public  PresignPartsResponse presignParts(UUID uploadSessionId, PresignPartsRequest request) {
    ...
    @Transactional
    public ConfirmUploadResponse complete(UUID uploadSessionId, CompleteMultipartRequest request) {
        UploadSession session = findMultipartSession(uploadSessionId);
    if(session.status != UploadSessionStatus.IN_PROGRESS) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "UPLOAD_ALREADY_FINALIZED");
        }
    ...
    @Transactional
    public void abort(UUID uploadSessionId) {
        UploadSession session = findMultipartSession(uploadSessionId);
    if(session.status != UploadSessionStatus.IN_PROGRESS) {
            return;
        }
